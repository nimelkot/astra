...
function renderCommandCenter() {{
    ...
    const healthFindings = health.findings.length
        ? health.findings.slice(0, 8).map(item => {{
            ...
            const cycleLabel = item.cycle_number ? ` ${{item.cycle_number}}` : '';
            return `<li><strong>${{escapeHtml(item.severity)}}</strong> · ${{escapeHtml(item.type)}}${{cycleLabel}} ${{escapeHtml(item.name || '')}} ${{escapeHtml(findingLocation(item))}}${{nodes ? '<br>' + nodes : ''}}</li>`;
        }}).join('')
